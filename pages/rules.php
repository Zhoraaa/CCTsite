...
<div class="content f-dir-column">
    ...
    <div class="gr-desc ctrl-j wrapper">
        ...
        <?php
        $RulesPrologue = $cctdb->query("SELECT * FROM rules_prologue");
    ...
    <ul class="wrapper ctrl-e">
        ...
        $get_r_groups = $cctdb->query("SELECT * FROM `rule_groups`");
        ...
        while ($r_groups = mysqli_fetch_assoc($get_r_groups)) {
            ...
            $get_rules = mysqli_query($cctdb, "SELECT * FROM `rules` LEFT JOIN `punishes` ON `rules`.`punish` = `punishes`.`id` WHERE `group`='$r_groups_id'");
        ?>
            <p>
                <!-- Название группы -->
                <h2><?= $r_groups["name"]; ?></h2>
